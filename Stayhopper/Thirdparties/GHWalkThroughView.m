...
//    [flowLayout setSectionInset:UIEdgeInsetsMake(0, 0, 0, 0)];
   ...
   // self.layout = flowLayout;
    ...
    _collectionView = [self viewWithTag:1];
...
//    _collectionView.showsVerticalScrollIndicator = NO;
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"GHWalkThroughPageCell" bundle:nil] forCellWithReuseIdentifier:@"cellIdentifier"];
...
-(float)getPageControlYPos
{
    return [Commons getTopSafeArea]+15.0;
}
- (void) orientFooter
{
    return;
    if (self.walkThroughDirection == GHWalkThroughViewDirectionVertical) {
        ...
        if (isRotated) {
            ...
            self.pageControl.transform = CGAffineTransformRotate(self.pageControl.transform, -M_PI_2);
            self.pageControl.frame = CGRectMake(0, [self getPageControlYPos], self.bounds.size.width, 20);
...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(40, [self getPageControlYPos], 145, 20)];
    self.pageControl.pageIndicatorTintColor = UIColorFromRGB(0x91e0fa);
    self.pageControl.currentPageIndicatorTintColor = UIColorFromRGB(0xfa4b4b);
   ...
   // self.pageControl.defersCurrentPageDisplay = YES;
    ...
    self.skipButton.frame = CGRectMake(self.bounds.size.width - 80-40, self.pageControl.frame.origin.y - ((30 - self.pageControl.frame.size.height)/2), 80, 30);
    ...
    self.skipButton.titleLabel.font = [UIFont fontWithName:FontMedium size:15];
    [self.skipButton setTitleColor:UIColorFromRGB(0x0082d9) forState:UIControlStateNormal];
...
-(void)setHeaderFrame
{
  //  self.pageControl.backgroundColor = [UIColor greenColor];
    CGRect frame = self.pageControl.frame;
    frame.origin.y = [self getPageControlYPos]+15;
