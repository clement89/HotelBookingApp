...
NSString * const MMbuttonTextColor = @"buttonColor";
NSString * const MMfont = @"font";
NSString * const MMvalueY = @"yValueFromTop";
NSString * const MMselectedIndex = @"selectedObject";
...
NSString * const MMButtonBGColor = @"buttonBGColor";
NSString * const MMtoolbarTitle = @"toolbarTitle";
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  id chosenObject = options[MMselectedIndex];
  ...
  UIColor *buttonTextColor = [[UIColor alloc] initWithCGColor:[options[MMbuttonTextColor] CGColor]];
  UIColor *buttonBGColor = [[UIColor alloc] initWithCGColor:[options[MMButtonBGColor] CGColor]];
  ...
  if (pickerViewFont==nil) {
    _pickerViewFont = [UIFont systemFontOfSize:13];
  }
  _pickerViewFont = [UIFont fontWithName:FontRegular size:16];
  ...
  _pickerViewBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(dismiss)];
    UIFont *toolfont =[UIFont fontWithName:FontMedium size:16];
    if (options[MMtoolbarTitle]) {
        float xOffset=60;
        ...
        lblTitle.textColor = buttonTextColor;
        lblTitle.textAlignment = NSTextAlignmentCenter;
        lblTitle.font=toolfont;

     //   lblTitle.font=[UIFont boldSystemFontOfSize:14];
        lblTitle.text=options[MMtoolbarTitle];
        UIBarButtonItem *typeField = [[UIBarButtonItem alloc] initWithCustomView:lblTitle];
        _pickerViewToolBar.items = @[flexibleSpace,cancel,flexibleSpace,typeField,flexibleSpace, _pickerViewBarButtonItem,flexibleSpace];
    ...
    [cancel setTitleTextAttributes:@{
         NSFontAttributeName: toolfont,
         NSForegroundColorAttributeName: [UIColor whiteColor]
    } forState:UIControlStateNormal];
    [_pickerViewBarButtonItem setTitleTextAttributes:@{
         NSFontAttributeName: toolfont,
         NSForegroundColorAttributeName: [UIColor whiteColor]
    } forState:UIControlStateNormal];
...
//   _pickerDoneButton = [[UIButton alloc] initWithFrame:CGRectMake(_pickerContainerView.frame.size.width - 90.0, 0.0, 70.0, 34.0)];
//   [_pickerDoneButton setTitle:@"Done" forState:UIControlStateNormal];
//    _pickerDoneButton.backgroundColor=buttonBGColor;
//    _pickerDoneButton.titleLabel.textColor=buttonTextColor;
//   
//    UIBarButtonItem* barButtonItem = [[UIBarButtonItem alloc] initWithCustomView:_pickerDoneButton];
//
//   _pickerViewToolBar.items = @[flexibleSpace, barButtonItem];
...
//  
  ...
  [_pickerView setShowsSelectionIndicator: TRUE];//YES];
...
- (id)selectedObject {
    ...
    return [NSString stringWithFormat:@"%ld",(long)[self.pickerView selectedRowInComponent:0]];
